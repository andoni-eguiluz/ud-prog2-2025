...
public class EjercicioFechasInterfaces {
	public static void main(String[] args) {
		...
		saga.ordenarDeOtroModo( new ComparadorNombres() );
		System.out.println( saga );
		...
		Iterator<Pelicula> iterador = saga.iterator();
		int i = 1;
		while (iterador.hasNext()) {
			System.out.println( "  Peli " + i + ": " + iterador.next() );
			i++;
		}
		
		System.out.println( "\nRecorrido con iterable sacando días de diferencia entre películas:" );
		i = 1;
		Pelicula pAnterior = null;
		for (Pelicula p : saga) {
			System.out.println( "  Peli " + i + ": " + p +
				((pAnterior==null) ? "" : " - Tras " + ChronoUnit.DAYS.between( pAnterior.getFecha(), p.getFecha() ) + " días") );
			i++;
			pAnterior = p;
		}
...
class ComparadorNombres implements Comparator<Pelicula> {
	@Override
